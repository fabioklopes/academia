...
def get_student_stats(student, start_date, end_date):
    ...
    for p in presencas_qs:
        # Regra de Negócio:
        # Terça-feira (weekday == 1): BOTH = 2 presenças, GI/NOGI = 1 presença.
        # Outros dias: Sempre 1 presença.
...
def aluno_cancelar_presenca(request, request_id):
    ...
    if attendance_request.status != 'PEN':
        ...
        # Delete the request so the date is released
...
def aluno_relatorio_presenca(request):
    ...
    for date_key, presencas_list in grouped_data.items():
        # Se houver mais de uma presença no dia, priorizamos a que não foi rejeitada, se houver.
        # Caso contrário, pegamos a primeira.
        # Mas a regra de negócio diz que terça pode ter 2 aulas.
        # Se for terça, precisamos ver se é BOTH ou GI/NOGI separados.
        
        is_tuesday = date_key.weekday() == 1
        
        # Se for terça e tivermos múltiplas entradas, precisamos consolidar?
        # A regra anterior já tratava GI/NOGI/BOTH.
        # Se o aluno foi nas duas aulas separadamente (GI e NOGI), teremos 2 registros.
        # Se foi BOTH, teremos 1 registro.
        
        # Vamos processar cada registro individualmente como antes, mas agora garantindo que não mostramos cancelados (já filtrado no exclude)
        # E se quisermos agrupar visualmente, podemos fazer isso no template ou aqui.
        # O pedido é "agrupamento de datas para que seja exibida apenas uma vez cada data".
        
        # Se tivermos 2 registros na terça (GI e NOGI), devemos mostrar 1 linha com soma?
        # Ou mostrar o status "Integral"?
        
        # Se tivermos GI (APR) e NOGI (APR) -> Integral (2 presenças)
        # Se tivermos GI (APR) e NOGI (REJ) -> Presente apenas na aula com Kimono (1 presença)
        
        # Vamos analisar os registros do dia para consolidar em uma única linha
        
        status_final = 'APR' # Default, ajustaremos
        motivo_final = ""
        qty_final = 0
        ...
        if is_tuesday:
            if apr_count > 0:
                # Verificamos se é BOTH ou se tem GI e NOGI
                ...
                # Se tiver rejeição, podemos mostrar o motivo da rejeição do primeiro
                if rej_count > 0:
                    ...
                    if first_rej and first_rej.rejection_reason:
                     ...
                     status_final = 'PEN' # Ou outro status se não tiver APR nem REJ (ex: PEN)
        else:
            # Dias normais
            if apr_count > 0:
                qty_final = 1
...
def professor_graduacoes(request):
    ...
    # We need graduations for the students on the current page
...
def relatorio_pedidos(request):
    ...
    if group_by_aluno:
        ...
        # Convert to list of tuples for pagination: [('Aluno A', [pedidos]), ('Aluno B', [pedidos])]
...
def relatorio_presenca(request):
    ...
    for (date_key, student), presencas_list in grouped_data.items():
        ...
        report_data.append({
            ...
            'student_obj': student, # Para uso no PDF
            'status': dict(AttendanceRequest.STATUS_CHOICES).get(status_final, status_final),
            'motivo': motivo_final,
            'qty': qty_final
        })
        
    # Ordenar por data e nome do aluno
    reverse = True if order == 'desc' else False
    ...
    if export_format == 'pdf':
        ...
        # Group data by student for the PDF
        students_data = {}
        
        # If a specific student is selected, ensure they are in the list even if no presences
        if aluno_id:
        ...
        # Process presences found in query (using the consolidated report_data)
